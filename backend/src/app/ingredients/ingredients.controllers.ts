...
class IngredientsControllers {
	...
	public async createIngredient(request: Request, response: Response): Promise<Response> {
		...
		return (await pool).query('INSERT INTO ingredients SET ?', [request.body]).then((value) => {
	...
	public async updateIngredient(request: Request, response: Response): Promise<Response> {
		const { id } = request.params;
		return (await pool)
			.query('UPDATE ingredients SET ? WHERE id = ?', [request.body, id])
			.then((value) => {
					...
					for (var i = 0; i < ingredientsInProduct.length; i++) {
						...
						await (await pool)
							...
							.then(async (date) => {
								...
								await (
									await pool
								).query('UPDATE ingredients SET amount = ? WHERE id = ?', [
									newAmount,
									idIngredient
								]);
							});
	...
	public async deleteIngredient(request: Request, response: Response): Promise<Response> {
		const { id } = request.params;
		return (await pool)
			.query('UPDATE ingredients SET active = false WHERE id = ?', [id])
			.then((value) => {
				return response
					.status(200)
					.json({ message: 'Ingredient eliminated successfully.' });
			});
	}
}
