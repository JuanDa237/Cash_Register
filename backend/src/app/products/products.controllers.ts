...
class ProductsController {
	...
	public async createProduct(request: Request, response: Response): Promise<Response> {
		...
		return (await pool)
			.query('INSERT INTO products SET ?', [request.body])
	...
	public async updateProduct(request: Request, response: Response): Promise<Response> {
		const { id } = request.params;
		return (await pool)
			.query('UPDATE products SET ? WHERE id = ?', [request.body, id])
			.then((value) => {
	...
	public async updateIngredientInProduct(
		...
		const { id } = request.params;
		return (await pool)
			.query('UPDATE detailProductsIngredients SET ? WHERE id = ?', [request.body, id])
			.then((value) => {
				return response
					.status(200)
	...
	public async deleteIngredientInProduct(
		...
		const { id } = request.params;
		return (await pool)
			.query('UPDATE detailProductsIngredients SET active = false WHERE id = ?', [id])
			.then((value) => {
				return response
					.status(200)
