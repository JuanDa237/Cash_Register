...
class CategoriesControllers {
	...
	public async createCategory(request: Request, response: Response): Promise<Response> {
		...
		return (await pool).query('INSERT INTO categories SET ?', [request.body]).then((value) => {
	...
	public async updateCategory(request: Request, response: Response): Promise<Response> {
		const { id } = request.params;
		return (await pool)
			.query('UPDATE categories SET ? WHERE id = ?', [request.body, id])
			.then((value) => {
	...
	public async deleteCategory(request: Request, response: Response): Promise<Response> {
		const { id } = request.params;
		return (await pool)
			.query('UPDATE categories SET active = false WHERE id = ?', [id])
			.then(async (value) => {
				await (await pool)
