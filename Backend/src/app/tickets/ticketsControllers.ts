...
class TicketsControllers {
    ...
    public async listTicketsInInterval(request: Request, response: Response): Promise<Response> {
        ...
        return (await pool).query("SELECT id, idClient, DATE_FORMAT(creationDate, '%d-%m-%Y') AS creationDate, total, homeDelivery, priceOfHomeDelivery FROM tickets WHERE creationDate >= ? AND creationDate <= ? AND idCompany = ?", [since, until, request.user.idCompany])
                        .then(dates => {
    ...
    public async listTicketsInYear(request: Request, response: Response): Promise<Response> {
        ...
        return (await pool).query("SELECT DATE_FORMAT(creationDate, '%m') AS creationDate, total, homeDelivery FROM tickets WHERE creationDate >= '?-01-01' AND creationDate <= '?-12-31' AND idCompany = ?", [year, year, request.user.idCompany])
                        .then(dates => {
                            return response.status(200).json(dates);
                        });
    }
    ...
    public async listTickets(request: Request, response: Response): Promise<Response> {
        return (await pool).query("SELECT id, idClient, DATE_FORMAT(creationDate, '%d-%m-%Y') AS creationDate, total, homeDelivery, priceOfHomeDelivery FROM tickets WHERE idCompany = ?", [request.user.idCompany])
                        .then(dates => {
                            return response.status(200).json(dates);
                        });
    }
    ...
    public async listProductsInTickets(request: Request, response: Response): Promise<Response> {
        return (await pool).query("SELECT * FROM productsInTickets WHERE idCompany = ?", [request.user.idCompany])
                        .then(dates => {
                            return response.status(200).json(dates);
                        });
    }
    ...
    public async getProductsInTicket(request: Request, response: Response): Promise<Response> {
        const { id } = request.params;
        return (await pool).query("SELECT * FROM productsInTickets WHERE idTicket = ? AND idCompany = ?", [id, request.user.idCompany])
                        .then(dates => {
    ...
    public async createProductInTicket(request: Request, response: Response): Promise<Response> {
        request.body.idCompany = request.user.idCompany;
        return (await pool).query("INSERT INTO productsInTickets SET ?", [request.body])
                        .then(value => {
