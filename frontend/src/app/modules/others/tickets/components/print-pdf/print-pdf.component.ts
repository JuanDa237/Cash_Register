...
export class PrintPdfComponent {
	...
	public print(): void {
		...
		const ticket = document.getElementById(this.elementId);
	...
	private generatePdfName(): string {
		...
		if (typeof this.ticket != 'undefined' && typeof this.client != 'undefined') {
			name = `${this.client.name}_${this.ticket.creationDate}`;
		} else {
